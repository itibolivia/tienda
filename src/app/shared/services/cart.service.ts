...
export class CartService {
  ...
  addCartProduct(payload: IProduct) {
    ...
    if (payload.status === 'out-of-stock' || payload.quantity === 0) {
      ...
      this.toastrService.success(`${payload.title} agregado al carrito`);
    } else {
        ...
        if (item.id === payload.id) {
          if (typeof item.orderQuantity !== "undefined") {
            if (item.quantity >= item.orderQuantity + this.orderQuantity) {
              ...
              this.toastrService.success(`${this.orderQuantity} ${item.title} agregado al carrito`);
            } else {
              this.toastrService.success(`No hay más cantidad de productos disponibles!`);
              this.orderQuantity = 1;
            }
          }
        }
...
// total price quantity
      ...
      if (item.id === payload.id) {
        if (typeof item.orderQuantity !== "undefined") {
          if (item.orderQuantity > 1) {
            ...
            this.toastrService.info(`Disminucación de cantidad para ${item.title}`);
          }
        }
      }
      return { ...item };
  ...
  removeCartProduct(payload: IProduct) {
    ...
    this.toastrService.error(`${payload.title} Quitado del carrito`);
    localStorage.setItem("cart_products", JSON.stringify(state.cart_products));
  };
  ...
  clear_cart() {
    const confirmMsg = window.confirm(
      "Está seguro que desea eliminar el ítem ?"
    );
    if (confirmMsg) {
      state.cart_products = [];
    }
